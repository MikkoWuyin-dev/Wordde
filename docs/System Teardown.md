...
8. Build canonical 66-book order once, from the first translation loaded, via `sortBooksInOrder` — seeded FROM the module-level `CANONICAL_BOOK_ORDER` spelling list rather than the dataset's raw spellings, so a divergent source name can never leak in regardless of which translation wins the parallel load race.
...
| Translation fails to load at boot | Corrupt/missing ZIP | `Promise.allSettled` isolates the failure: boot proceeds with the rest, the failed translation stays unavailable (reads return empty, no fallback — RI-014), failures logged with names and errors (RI-044). Total failure (zero loaded) → `preloadAllTranslations` throws → error state via boot `.catch` | No UI surface shows *which* translation failed; if it is the current one it just renders empty (known limitation, product decision pending) |
